...
export function BuyModal({ isOpen, onClose, listing }: BuyModalProps) {
  ...
  const isNative =
    listing?.paymentToken.toLowerCase() === BTC_ADDRESS.toLowerCase();
  ...
  const paymentSymbol = listing ? getPaymentSymbol(listing.paymentToken) : "";
  ...
  useEffect(() => {
    if (error && (step === "approving" || step === "buying")) {
  ...
  async function handleBuy() {
    ...
    try {
      if (isNative) {
        ...
        setPhase("buy");
        setStep("buying");
        buyListing(listing.listingId, listing.price, true);
      } else if (alreadyApproved) {
        // Router already has sufficient allowance — skip approval step
        setPhase("buy");
        setStep("buying");
        executeBuy(listing.listingId);
      } else {
        // ERC-20: approve router first, then buy after tx confirmation
        setPhase("approve");
        setStep("approving");
        approveTokenForBuy(listing.paymentToken, listing.price);
      }
    } catch (err: unknown) {
